...
async function robot() {
  ...
  async function fetchImagesOfAllSentences(content) {
    for (let sentenceIndex = 0; sentenceIndex < content.sentences.legth; sentenceIndex++) {
	    let query

    if (sentenceIndex ===0) {
      query = `${content.searchTerm}`
    } else {
	query =	`${content.searchTerm} ${content.sentence[sentenceIndex].keywords[0]}`
    }

	console.log(`> [image-robot] Querying Google Images with: "${query}"`)

      content.sentence[sentenceIndex].images = await fetchGoogleAndReturnImagesLinks(query)
...
module.exports = robot
 ...
 convert-images
	await fetchImagesOfAllSentences(content)
	await downloadAllImages(content)
	await contentAllImages(content)
 ...
 master
  ...
  async function downloadAllImages(content) {
    ...
    for (let sentenceIndex = 0; sentenceIndex < content.sentences.length; sentenceIndex++) {
      ...
      for (let imageIndex = 0; imageIndex < images.length; imageIndex++) {
        ...
        try {
          if (content.downloadedImages.includes(imageUrl)) {
            throw new Error('Imagem j� foi baixada')
          }
  ...
  async function downloadAndSave(url, fileName) {
	...
	async function convertAllImages(content){
		for (let sentenceIndex = 0;sentenceIndex < content.sentences.lenght; sentenceIndex++){
